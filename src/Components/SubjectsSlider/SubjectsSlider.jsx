...
import { getSubjects } from "../../Helper/helper"
  ...
  return (
    <div className="w-full flex flex-col rtl">
     ...
     <h2 className="text-[24px] leading-[32px] font-bold ">المواد الدراسية</h2>
      ...
      <div
        ref={sliderRef}
        className={`flex overflow-x-auto gap-4 pb-4 ${isDragging ? "cursor-grabbing" : "cursor-grab"}`}
      ...
      >
        ...
        {subjects.map((subject) => {
          // Round the progress value to nearest 10 (two digits)
        //   const roundedProgress = roundProgress(subject.progress)

          return (
            <div
              key={subject.id}
              className="w-72 lg:min-w-96 flex-shrink-0 rounded-xl bg-white p-6 shadow-sm border border-gray-100 flex flex-col"
